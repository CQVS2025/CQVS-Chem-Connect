...
/**
 * Shape of a product after we've normalised either the live API row or the
 * static fallback row. Kept in lockstep with the original page's `useMemo`.
 */
...
interface ProductsClientProps {
  /**
   * Server-rendered seed list (active products from Supabase). Used as the
   * initial render so Googlebot sees a populated catalog without executing
   * JS. The `useProducts()` hook then refreshes from the API on the client.
   */
...
export function ProductsClient({ initialProducts }: ProductsClientProps) {
  ...
  // The hook keeps client-side product data fresh after navigation. While
  // it's loading on first paint we use the SSR seed instead of skeletons -
  // this is what makes the page actually crawlable.
  ...
  const products = useMemo<NormalisedProduct[]>(() => {
    ...
    // Before the API call resolves, prefer the SSR seed. Only fall back to
    // the bundled static catalogue if SSR returned nothing (e.g. Supabase
    // env vars missing in dev).
  ...
  // Skeleton state only when we have NEITHER the API result NOR the SSR
  // seed. With SSR working this is effectively never reached for crawlers
  // or returning users.
  const showSkeleton = isLoading && initialProducts.length === 0
